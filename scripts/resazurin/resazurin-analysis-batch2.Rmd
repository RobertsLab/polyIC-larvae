...
ADD: Metabolic rate ~ mortality; ending mortality in TPC format 
...
plot1c<-full_data%>%
  ...
  scale_colour_manual(values=c("control"="darkgray", "treated"="purple4"))+
  scale_fill_manual(values=c("control"="darkgray", "treated"="purple4"))+
  theme_classic()+
  xlab("Hour");plot1c

ggsave(plot1c, filename="figures/resazurin/batch2_metabolism_treatment.png", width=7, height=5)
...
plot5a<-ggplot(end_data, aes(x = temperature, y = value.mm)) +
  geom_point(alpha = 0.6, position = position_jitter(height = 0.03), colour="darkgray") +
...
ggsave(plot5a, filename="figures/resazurin/batch2-tpc.png", width=6, height=5)
```

Analyze effect of temperature. 

```{r}
tpc_model<-lmer(value.mm ~ temperature + (1|holding_tank) + (1|spat_treatment:tank), data=end_data)
