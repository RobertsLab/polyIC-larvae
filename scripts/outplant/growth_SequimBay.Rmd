...
training_data<-read_csv(file="https://raw.githubusercontent.com/RobertsLab/project-gigas-conditioning/refs/heads/main/data/outplanting/GoosePoint/growth_GoosePoint.csv")%>%
  mutate(date=as.factor(date))%>%
  filter(date %in% c("20240909", "20251009", "20260522"))
...
```{r}
data<-data%>%
  filter(Predicted_Volume_Poly<100000)
...
```{r}
plot5<-data%>%
  ggplot(aes(x=as.factor(date), y=Predicted_Volume_Poly, fill = treatment)) +
  #facet_wrap(~treatment)+
  geom_boxplot(width=.5, outlier.shape= NA, position = position_dodge(width = 0.4)) +
  geom_point(pch = 21, position=position_jitterdodge(dodge.width=0.4), alpha=0.3) +
  xlab("Date") + 
  ylab("Volume")+
  theme_classic() +
...
```{r}
data%>%
  dplyr::select(date, tag, oyster_no, treatment, Predicted_Volume_Poly)%>%
  mutate(treatment=if_else(treatment=="Control", "Control", 
                           if_else(treatment=="Treated", "Immune PolyIC", NA)))%>%
  write_csv("output/sequim_polyic_growth.csv")
```

# Survival 

```{r}
surv<-read_xlsx("data/outplant/sequim/survival.xlsx")
```

Calculate proportion alive at each subsequent time point. 

```{r}
surv_p<-surv%>%
  arrange(outplant_tag, date)%>%
  group_by(outplant_tag)%>%
  mutate(start=first(alive))%>%
  mutate(prop_survival=1-((start-alive)/start))
```
...
```{r}
plot7<-surv_p%>%
  ggplot(aes(x=as.factor(date), y=prop_survival, fill = treatment, group=interaction(date,treatment))) +
  ylim(0,1)+
  geom_boxplot(width=.5, outlier.shape= NA, position = position_dodge(width = 0.4)) +
  geom_point(pch = 21, position=position_jitterdodge(dodge.width=0.4)) +
  xlab("Date") + 
...
ggsave(plot7, filename="figures/outplant/sequim_survival.png", width=4, height=4, units="in")
```
...
```{r}
surv_p%>%
  dplyr::select(date, outplant_tag, treatment, prop_survival)%>%
  mutate(treatment=if_else(treatment=="Control", "Control", 
                           if_else(treatment=="Treated", "Immune PolyIC", NA)))%>%
  write_csv("output/sequim_polyic_survival.csv")
```
