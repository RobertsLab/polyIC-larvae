...
```{r}
surv<-surv%>%filter(!c(date=="20250312" & hours=="5")) #remove NAs from timepoint that wasn't recorded
...
emm<-emmeans(surv_model, ~hours|temperature)
pairs(emm)
```
...
summary(surv_model)
Anova(surv_model, type = "II")

#check for overdispersion
library(DHARMa)

sim <- simulateResiduals(surv_model)
testDispersion(sim)
...
emm <- emmeans(surv_model, ~ treatment | temperature, type = "response")

# Pairwise comparisons of treatments within each temperature
pairs(emm)
...
# If you also want to compare temperatures within each treatment:
emm_temp <- emmeans(surv_model, ~ temperature | treatment, type = "response")
pairs(emm_temp)
```
...
```{r}
# Generate predictions across temperature and treatment from our model
preds <- ggpredict(surv_model, terms = c("temperature [all]", "treatment"), type = "fixed")

# Inspect predictions
head(preds)

# Plot model-based predictions + confidence intervals
plot4 <- ggplot() +
  # Raw mortality points, jittered for visibility
  geom_point(data = surv %>% filter(hours=="24"),
             aes(x = temperature, y = mortality, color = treatment),
             position = position_jitter(width = 0.3, height = 0.03),
             alpha = 0.6, size = 2) +
  # Model-predicted lines
  geom_line(data = preds, aes(x = x, y = predicted, color = group, group = group), linewidth = 2) +
  # Manual colors
  ...
  scale_fill_manual(values = c("control"="cyan3", "treated"="coral1"),
                    name = "Parental \nTreatment") +
  # Labels and theme
  labs(y = "Model Predicted Mortality", x = "Temperature") +
  # Manual significance text
  geom_text(aes(x=6, y=0.65, label="*"), size=12, inherit.aes = FALSE) +
  geom_text(aes(x=7, y=0.8, label="*"), size=12, inherit.aes = FALSE) +
  theme_classic(base_size = 16); plot4
