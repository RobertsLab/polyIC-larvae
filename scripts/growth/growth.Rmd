...
plot1 <-ggplot(growth, aes(x = age_days, y = length.mm, color = treatment, fill=treatment)) +
  ...
  scale_color_manual(values = c("control" = "cyan3", "treatment" = "coral1"), labels=c("control", "treated")) +
  scale_fill_manual(values = c("control" = "cyan3", "treatment" = "coral1"), labels=c("control", "treated")) +
  geom_text(x=93, y=7, label="*", size=8, inherit.aes=FALSE)+
  geom_text(x=59, y=5, label="*", size=8, inherit.aes=FALSE)+
...
#use a transformation to meet assumptions 

model<-lm(log10(length.mm) ~ treatment * as.factor(age_days), data=growth)
car::Anova(model, type = "II")
qqPlot(residuals(model))
...
View post hoc tests. 
```{r}
emm<-emmeans(model, ~treatment|as.factor(age_days))
pairs(emm)
