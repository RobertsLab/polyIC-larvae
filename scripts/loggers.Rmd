...
Field deployment (Sequim Bay)
```{r}
# Dates to filter out
dates_to_filter <- as.Date(c("2025-07-10"))
...
temp_plot<-tidbits%>%
  ...
  ylab("Temperature (°C)")+
  xlab("Date")+
  ggtitle("Sequim Bay Outplant")+
  ylim(-10,40)+
  theme_classic(); temp_plot

ggsave(temp_plot, filename="figures/environmental/PolyIC_SequimBay.png", width=6, height=4)
```
...
dates_to_filter2 <- as.Date(c("2025-07-10"))

temp_plot<-tidbits%>%
  filter(group=="experiment")%>%
  mutate(day=as.Date(DateTime))%>%
  filter(!day<dates_to_filter)%>%
  filter(!day>dates_to_filter2)%>%
